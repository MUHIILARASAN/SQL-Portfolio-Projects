...
-- Questions--
-- 1 Retrieve all the stations in the East Region;
...
select hc.HostName, sh.ShowName from Hosts hc
join Shows sh on hc.HostID = sh.HostID
where hc.HostName = 'Vrajesh Hirjee';

-- Count the number of awards each show has won?

select count(a.AwardID) as total_awards, s.ShowID, s.ShowName from Shows s
...
from shows order by LaunchDate desc;

-- 7) Find the total count of shows for each host?

select HostName, sum(Showcount) as total_show_count
from Hosts
group by Hostname;

-- 8) How the online presence platforms with their links?
